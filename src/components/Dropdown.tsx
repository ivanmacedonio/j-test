...
export default function DropdownComponent({ setStatus }: DropdownProps) {
  const [auxStatus, setAuxStatus] = useState<string>("");
  function handleStatus(prop: string) {
    setStatus(prop);
    setAuxStatus(prop);
  }
  return (
      ...
      <div>
        <Menu.Button
          className={`inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 ${
            auxStatus === "Alive" ? "bg-green-400" : "bg-red-400"
          }`}
        >
          {auxStatus !== "" ? <p>{auxStatus}</p> : "Status"}
          <ChevronDownIcon
